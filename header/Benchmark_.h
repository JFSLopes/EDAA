...
struct BenchmarkConfig {
    ...
    std::vector<int> pqVertexCounts  = { 1000, 2000, 5000, 10000, 20000, 30000, 40000 };
...
struct BenchmarkRow {
    ...
    std::uint64_t pqInserts = 0;
    std::uint64_t pqDeletes = 0;
    std::uint64_t pqUpdateKeys = 0;
...
public:
    ...
    double timedAverage(const std::function<void()>& fn, int totalRuns = 3, int warmupRuns = 1) const;
    BenchmarkConfig cfg;
    void record(BenchmarkRow row);

    TimedPQResult timedPQAverage(const std::function<void()>& fn, int totalRuns = 5, int warmupRuns = 2) const;
